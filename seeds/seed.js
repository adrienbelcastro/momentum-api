...
async function getImageURL() {
  ...
  try {
    ...
    const imageURL = res.data.results[1].urls.regular;
    const imageURL2 = res.data.results[2].urls.regular;
    const imageURL3 = res.data.results[3].urls.regular;
    const imageURL4 = res.data.results[4].urls.regular;
    const imageURL5 = res.data.results[5].urls.regular;
...
const seedData = async function (connection) {
  try {
    console.log("seedArticles function is called"); // Add this line

    await connection.query(dropArticleTableSql);

    console.log("Dropped article table"); // Add this line for debugging

    const { imageURL, imageURL2, imageURL3, imageURL4, imageURL5 } =
      await getImageURL();

    await connection.query(createArticleTableSql);
    await connection.query(insertArticleTableSql, [
      Object.values({
      ...
      Object.values({
        ...
        title: "Exploring Exotic Flavors: A Culinary Journey Around the World",
        introduction:
    ...
    await connection.query(insertUserTableSql, [
      [
        "0bae167c-42e5-11ee-be56-0242ac120002",
        "test",
        "test@email.com",
        "1234",
        "905-456-3214",
